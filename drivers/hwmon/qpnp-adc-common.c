...
static const struct qpnp_vadc_map_pt adcmap_atm_threshold[] = {
	{-400,	1759},
	{-390,	1756},
	{-380,	1753},
	{-370,	1749},
...
int32_t qpnp_adc_scale_ambient_therm(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
{
	int64_t ambient_voltage = 0;

	qpnp_adc_scale_with_calib_param(adc_code,
			adc_properties, chan_properties, &ambient_voltage);

	adc_chan_result->measurement = ambient_voltage;

	return qpnp_adc_map_temp_voltage(
			adcmap_atm_threshold,
			ARRAY_SIZE(adcmap_atm_threshold),
			ambient_voltage,
			&adc_chan_result->physical);
}
EXPORT_SYMBOL(qpnp_adc_scale_ambient_therm);
